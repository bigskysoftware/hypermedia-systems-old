...
* A host, represented by a domain name (in this case `hypermedia.systems`)
...
Perhaps if you are doing something in big data, you'd like to use Python, which has tremendous support for that
domain.

Perhaps if you are doing AI work, you'd like to use Lisp, leaning on a language with a long history in that
area of research.
...
cacheability of responses for future requests of the same resource.  This allows both clients as well as intermediary
...
again, the uniform interface of REST has proven to be extremely flexible: despite a potentially radical new layout
...
the hypermedia model.  In a later chapter, we will elaborate on what scripting to augment the underlying hypermedia
