...
we wish we could write this book for every possible stack out there, in the interest of simplicty (and sanity) we can only pick one.
...
With this stack we will be rendering HTML _on the server_ to return to clients, rather than producing JSON.  This
