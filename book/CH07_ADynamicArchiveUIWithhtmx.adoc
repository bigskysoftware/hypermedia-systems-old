...
For completeness, here is the CSS we'll use for this progress bar:
...
Like much else with hypermedia, the code is readable and not overly complicated.
...
As nice as this UI is, there is one minor annoyance: as the progress bar updates it "`jumps`" from one position to the next.  This feels a bit like a full page refresh in web 1.0 style applications. Can we fix this? 
...
So why isn't that transition being applied in our current UI?  Because, in our example, htmx is
...
with their SPA counterparts: it is hard to use CSS transitions without some JavaScript.

The good news: htmx has a way to rectify this situation. 
...
So, what is this strange little dance supposed to achieve?  Well, if an element
...
alternatives. Currently, a progress bar shows the progress of
