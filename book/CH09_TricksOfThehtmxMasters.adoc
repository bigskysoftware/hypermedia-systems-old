...
We will go over some the more advanced attributes in htmx, as well as expand on the advanced details of some attributes we
have already used.
...
By understanding all the features and techniques in this chapter, you should will be able to pull off extremely
sophisticated user interfaces using only htmx and perhaps a small bit of hypermedia-friendly client-side scripting.

== htmx Attributes

Thus far we have, we've used about fifteen different attributes from htmx in our application.  The most important ones
have been:
...
In addition to this _request header_, htmx also supports a _response_ header also named `HX-Trigger`.  This response header
