...
In addition to basic UI elements (`<view>`, `<text>`, `<image>`), HXML includes elements to implement mobile-specific UIs.
This includes layout patterns (`<screen>`, `<list>`, `<section-list>`) and input elements (`<switch>`, `<select-single>`, and `<select-multiple>`).
...
- Navigation actions (`push`, `back`, etc) enable navigating between the screens of a mobile app
