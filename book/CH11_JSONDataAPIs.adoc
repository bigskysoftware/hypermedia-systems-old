...
In the case of the JSON API, no such path is needed: the JSON API "`just is,`" it doesn't need to provide a
hypermedia representation for creating a new contact.  You simply know where to issue a `POST` to create a contact,
