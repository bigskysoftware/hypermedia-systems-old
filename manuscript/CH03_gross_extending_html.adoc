...
* From a technical perspective, all the updates are done with the `POST` HTTP action.  This despite the presence of more
  logical actions HTTP actions like `PUT` and`DELETE` for some of the operations.
...
you adopt a hypermedia-oriented library like htmx.  To understand conceptually how htmx allows us to address the perfectlt
valid UX concerns of Web 1.0 style applications, let's revisit the anchor tag from Chapter 1 and look at it
in depth:

This simple hyperlink, when interpreted by a browser, creates a web link to the Manning website:
...
Let's break down exactly what this link tells a browser to do, this time in detail:
...
So there are four aspects of a simple hypermedia link like this, with the last three being the mechanic that distinguishes
a hyperlink from "normal" text.

Now let's take a moment and think about generalizing this hypermedia mechanic within HTML.  There is no rule saying that
hypermedia can _only_ work this way, after all!

The first thing to think about is: why are anchor tags so special?  What if it were the case that _any_ element could issue
a HTTP request.  For example, shouldn't `button` elements be able to issue requests too?  It seemed silly to have to
wrap a form tag around a button to make deleting contacts work.  Why should only anchor tags and forms be able to issue
requests?

And, while we are on the topic, what's so special about clicking (in the case of anchors) or submitting (in the case of
forms)?  Those are just one of many, many events that are fired by the DOM, after all.  Events like mouse down, or key up, or
blur are all events you might want to use to issue an HTTP request. Why shouldn't these other events be able to trigger
requests as well?

Going further, why does plain HTML only give us access to the `GET` and `POST` actions of HTTP?  HTTP _stands_ for
...
That is certainly ridiculous, and it should be fixed!

Finally, and this is perhaps the most important conceptual extension we will make to  HTML: why should it be necessary to
replace  the _entire_ screen when an HTTP request is made by an element in HTML?  Normal HTML actually has a mechanism for
...
HTML-driven applications function more like a Single Page Application, where only part of the DOM is updated by a
given user interaction or network request.

If we were to address these four issues, we would be extending HTML beyond its normal capabilities, and we would
be doing so _entirely within_ the normal, hypermedia model of the web.  Note that none of the extensions involve
going outside the normal exchanging-HTML-over-HTTP found in Web 1.0 applications.  Rather, the all four are simply
generalizations of existing functionality already found within HTML.

== Extending HTML as a Hypermedia with htmx

It turns out that there are JavaScript libraries that extends HTML in exactly this manner.  It may seem somewhat ironic,
...
takes this hypermedia-approach, but it is perhaps the purest in the pursuit of extending HTML as a hypermedia.  It focuses
on the four issues discussed above and attempts to incrementally address each one, without introducing a significant
amount of additional conceptual infrastructure.
...
From a pratical, getting started perspective, htmx is a simple, dependency-free and stand-alone library that can be
...
That's all it takes to install htmx!  This is in the spirit of the early web when you could simply include a script tag
and things would work.  It still feels like magic, even today!  (Perhaps especially today, if you are used to more
elaborate build infrastructure!)

Of course, you may not want to use a CDN, in which case you can download htmx to your local system and adjust this
script tag.  Or, you may have a more sophisticated build system that automatically installs dependencies.  In this case you
...
And, unlike the vast majority of JavaScript libraries, htmx does not require you, the user, to actually write any
JavaScript!
...
That being said, an important difference between the HTTP responses to normal anchor and form driven requests and to
htmx-powered requests like the one made by this button is that, in the case of htmx, responses are often only _partial_
bits of HTML.

In htmx-powered interactions we are often not replacing the whole document, so it is not necessary to transfer an entire
