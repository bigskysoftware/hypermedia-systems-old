...
HTML-based application.  But we aren't done yet!  Let's keep pushing the envelope with hypermedia and see just how
far we can get with this approach.  We _will_ do some client-side scripting, the standard way to add more advanced
UX patterns to a web application, in our application a bit later on, but we still have quite a bit of pure
hypermedia functionality in front of us !
