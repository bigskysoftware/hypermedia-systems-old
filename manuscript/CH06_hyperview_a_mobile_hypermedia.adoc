...
This is something we're familiar with from the previous section in this chapter.
However, the `<text>` element contains a new child element, `<behavior>`.
The `<behavior>` element represents an interaction on the parent `<text>` element.
It contains several attributes that defines what should happen (the action) and when it should happen (the trigger).
First, the `trigger` attribute is set to `press`.
As you can imagine, that means the action of this behavior will happen when the `<text>` element is pressed.
Other triggers include:

  - `longPress`: triggers the action when the user presses and holds on the element. This is useful for providing extra actions for power-users.
  - `visible`: triggers the action when the element is visible in the viewport. This is useful to load extra content when scrolling through a list.
  - `refresh`: triggers the action when the user does a "pull-to-refresh" gesture on the element. Most commonly used on `<body>`, `<list>`, and `<section-list>` elements.
  - and many more for interactions with inputs

Next, the `action` attribute is set to `push`.
This tells the Hyperview client that when 
