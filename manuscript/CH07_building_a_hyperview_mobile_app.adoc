...
* We will take our existing Contacts web app, and transform it to a mobile app by tweaking the backend and using the Hyperview client.
* To enhance our app and make it more native, we will extend the Hyperview client with custom components and behaviors.
* Finally, we will make our Flask backend serve both the web app and mobile app using content negotation and other techniques.
...
.Almost-zero Configuration in Hypermedia Applications
----
...
We now have a fully-featured native mobile version of our Contacts app.
...
----
<1> When clicked, rompt the user to call the given phone number
...
Remember that behaviors are interactions are defined in HXML.
...
<2> Re-start the Expo server to run the mobile app in the iOS simulator.
...
I took the liberty of implementing a second custom action, "open-email".
As you can guess, this action will open a system-level email composer to let the user send an email to their contact.
I didn't include the code for this action, but if you understand how "open-phone" works, you can implement "open-email" too.
In fact, the `react-native-communications` library exposes a function called `email()`, I think you can guess what it does.
...
We wrapped an existing 3rd party library, and read attributes set on the `<behavior>` element to pass along data to the library.
After re-starting our demo app, our client has new capabilities we can immediately utilize.

=== Adding Toast Messages
The custom actions added in the previous section are what I call "system actions".
System actions trigger some UI or capability provided by the device's OS.
But custom actions are not limited to only interacting with system-level APIs.
Remember, the functions that implement actions can run arbitrary code, including UI-displaying code.
We're going to use that capability to trigger a new UI element: confirmation toasts.

Our Contacts web app shows confirmation toasts to give the user feedback on successful actions, like deleting or creating a contact.
We did not include this feature in the initial version of our mobile Contacts app, so let's add it now.
Ideally, the confirmation toast fades in at the top of the device display, then fades out after a few seconds.
This is another opportunity to use an open-source library, `react-native-root-toast`.
To use this library, just run `yarn add react-native-root-toast` and restart the demo app with `yarn start`.
...
export default {
  ...
  callback: (behaviorElement) => { <3>
    ...
    if (message != null) {
      Toast.show(message, {position: Toast.positions.TOP, opacity: 1}); <4>
    }
  },
};
----
...
<2> The name of the action
<3> The callback that runs when the action triggers
<4> Pass the message to the function that displays the message.
