...
TODO: summarize, talk about multiple pagination requests.
...
image::../images/screenshot_hyperview_detail.png[]
...
But that wouldn't quite feel right from a user-experience perspective.
...
So instead of pushing a new screen, it would be better to replace the current screen with the editing UI.
That means we need to add a button and behavior that use the `reload` action.
Let's add this to the header of the contact details screen.
...
This will ensure that data from user inputs will be included in any HTTP requests to our backend.
...
I ommitted the code for the contact's phone number and email address, because they follow the same pattern as the first and last name.
...
That depends on how the validitiy of the form data:
...
Let's make some changes to the existing `contacts_edit_post()` view in the Flask app.
...
<3> On success, render the form fields template, but pass a `saved` flag to the template
...
The result?
When the user hits "Save", our backend persists the new contact data, and the screen switches back to the read-only view.
Since we fetch the Details screen from the backend, it shows the edits the user just made.
...
There's one more enhancement we can make to this UI.
You have

Finally, let's talk about "Cancel", the last button in the actions section of the edit screen.
Cancel will switch the UI back to the view-only mode of the screen.
This is done using another `reload` action, this time reloading the current screen with the contact details screen.
(From the detail screen, the user could hit "Edit" to again switch modes by reloading the edit screen).
