...
* Extending the mobile app with custom behavior actions
* Extending the mobile app with custom components
...
But in the process of porting the backend to serve our mobile app, we gave up the web app.
Indeed, if you tried to visit `http://0.0.0.0:5000` in a web browser, you would most likely see a jumble of text and XML markup.
That's because web browsers don't know how to render plain XML into a UI, and they certainly don't know how to interpret HXML's semantics for behaviors.
It would be great if we could use the same backend to server both our web app and mobile app.
After all, nearly all of the model logic and HTTP endpoints are identical.
Is it possible?

The answer is yes!
We just need to know when the server should response with HTML or HXML, based on the Hypermedia format preferred by the client.
Luckily, HTTP comes to our rescue with the `Accept` request header.
The `Accept` header indicates the content types the client is able to handle.
Web browsers will list `text/html` as the preferred content type in the `Accept` header.
The Hyperview client lists `application/vnd.hyperview+xml` as the preferred content type.
This different content type
Our backend server can parse the `Accept` header and decide what type of response to send back (HTML or HXML) based on the content type desired by the client.

There are two approaches for branching 

=== TODO Approach 1: Template switching
...
=== TODO Approach 2: View switching


== Extending the Client

_Note to Editor: should we consider turning this section into a separate chapter? The concepts and techniques are distinct from the other sections (backend/HXML work vs using Javascript to add support for new components)._
