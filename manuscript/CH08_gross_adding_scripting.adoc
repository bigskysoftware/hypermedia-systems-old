...
Right now, clicking the `Delete` link uses `hx-confirm` to show a confirmation dialog. `hx-confirm` is a pretty rudimentary solution, relying on the browser built-in `confirm()` function. We're going to replace it with SweetAlert2 (https://sweetalert2.github.io), which we'll hook up to our button with Alpine.

Here's the Delete button as we left it last time:
...
----
<1> This is what we will replace.

We need to import the library into our page.
...
<a href="#" hx-delete="/contacts/{{ contact.id }}"
    hx-swap="outerHTML swap:1s"
    x-data="{ message: 'Are you sure you want to delete this contact?' }" <1>
    @click="Swal.fire(message).then(result => { <2>
      if (result.isConfirmed) { <3>
        // ???
      }
    })"
    hx-target="closest tr">Delete</a>
...
<2> When clicked, call into SweetAlert2
<3> Check the status
