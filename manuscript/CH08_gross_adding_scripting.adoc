...
<div x-data="{ open: false }">
  ...
  <div role="menu<1>" id="contents" x-show="open"
    ...
    x-effect="open ? $el<5>.firstElementChild().focus()<6> : null"
...
Our menu component has a lot of attributes that will be repeated in every item of the table. This is hard to maintain when manually writing HTML and increases payload sizes when generating it via a template. We can use an additional feature of the `x-bind` attribute to rectify this:
...
We can then reference it in HTML as follows:
...
Another useful tool in factoring Alpine code is that we can call functions in `x-data` as follows:
...
While previous two examples are JavaScript-oriented, _hyperscript (https://hyperscript.org[], the underscore is part of the name) is a entire new scripting language for
...
The implementation of our example button in hyperscript is similar, in some ways, to the VanillaJS implementation: we will
...
<div>
    <button 
      _="on click show the next <span /> with display:inline" <1>
    >
      Expand
    </button>
...
<2> This element is what `the next <span />` evaluates to. We'll leave you to figure out the rest.

Seasoned JavaScript programmers are often suspicious of _hyperscript: There have been many "natural language programming" projects that usually target non-programmers and beginner programmers, assuming that being able to read code will give you the ability to write it as well. (The authors' views on the usefulness of natural language for teaching programming are nuanced and out of scope for this book). _hyperscript is openly a programming language, in fact, its syntax is inspired in many places by the speech patterns of web developers. In addition, _hyperscript's readability is achieved not through complex heuristics or NLP, but common parsing tricks and a culture of readability.
...
[source,hyperscript]
----
-- Double dash marks comments.
-- The top level of a _hyperscript command is made up of _features_.
on -- One kind of feature is an event listener, introduced by the keyword "on",
  click -- ... followed by the event name,
  -- ... and a sequence of _commands_.
  show -- The show command shows an element.
    the -- The prefix "the" does nothing.
    next -- "next" before a CSS expression (see later) finds the first match
         -- after the element the script is written on (referred to as "me").
    <span /> -- CSS selectors are wrapped in "<" and "/>".
  with -- This "with" is part of the "show" command's syntax.
    *display:inline -- The asterisk is used to refer to CSS properties.
                    -- Here, we specify how to show the element
                    -- (options are display, visibility or opacity).
----
...
----
<input id="search" name="q" type="search" placeholder="Search Contacts"
  _="on keydown <1>[shiftKey<2> and<3> code is<4> 'KeyS']
       -- ...">
----
