...
Scripting has been a massive multiplier of the Web's capabilities. Through its use, Web application authors are not only able to enhance their hypertext-based websites, but also create full-fledged client-side applications that can compete with native apps in how they work (although they don't always win when these competitions). In other terms, the Web became a distribution medium for non-REST apps in addition to being a RESTful system.

When it's not used as a replacement for the RESTful architecture provided by the Web, however, scripting is extremely useful in Hypermedia Driven Applications as well. You are scripting in a way compatible with HDAs if:

* The main data format exchanged between server and client is hypermedia, the same as it would be with no scripting.
...
Simply listing "best practices", however, is rarely convincing or edifying (and, frankly, it is often boring). So, we will instead frame them around shiny tools that work well for scripting in a HDA. We will use each of these tools to add a feature to ContactApp:
...
The important idea in the implementation of each of these features is that while they are implemented entirely client-side, they don't exchange information with the server using, for example, JSON. This constraint is what will keep the features within the bounds of a Hypermedia Driven Application.
...
Today, we have a variety of _transcompilers_ (often shortened to _transpilers_) that convert another language to JavaScript, such as TypeScript, Dart, Kotlin, ClojureScript, F#. There is also the WebAssembly bytecode format, which is supported as a compilation target for C, Rust, and the WASM-first language AssemblyScript. However, most of these are not geared towards an HDA-compatible style of scripting --- compile-to-JS languages are often paired with SPA-oriented libraries (Dart and AngularDart, ClojureScript and Reagent, F# and Elmish), and WASM is currently mainly geared toward linking to existing C/C++ libraries from JavaScript.
...
Installation of vanilla JS couldn't be easier: you don't have to. You can just start writing JavaScript in your web application, and it will simply work.
...
Unfortunately, careless use of CSS selectors in JavaScript is the cause of _jQuery soup_, where:
...
The fix for this issue is trivial (change `.textContent` to `.value`), but it's not hard to see the burden of synchronizing markup changes and code changes across multiple files increasing in larger components or across a whole page.

image::../images/separation-of-concerns-expectation-v-reality.png[]

The tight coupling between files in this simple example suggests that separation between HTML and JavaScript (and CSS) is often an illusory separation of concerns. In Contact.app we are not _concerned_ with "structure", "styling" or "behavior"; we are concerned with collecting contact info and presenting it to users. SoC in the way it's formulated in web development orthodoxy is not an architectural guideline, but a stylistic choice that can even become a hindrance.

It turns out that we are not alone in thinking that separation of concerns (as understood by web developers) has been oversold. Consider the following technologies:
...
The `addEventListener` method in JavaScript is, in a way, a sort of "monkey-patching". It functions in much the same way
for event listeners as `define_method` in Ruby does for methods:
...
Interestingly, "monkey-patching" actually used to be the default way of adding methods to things in JavaScript, by modifying a function's `prototype`. After a conventional class syntax was added to JavaScript in ES2015, modifying the `prototype` of a function has become increasingly discouraged and "monkey patching" less and less common. No such advancement has been made for _event listeners_, however, leaving us stuck with `addEventListener`.
...
JavaScript and Locality of Behavior don't seem to mesh as well as we want them to, but the situation is not hopeless. It's important to be aware that LoB does not require behavior to be _implemented_ at the use site, but merely _invoked_ there. Keeping this in mind, it's possible to improve LoB while writing JS in a separate file, provided we have a reasonable system for structuring our JavaScript.
...
We won't reproduce all of the guidelines here, but here are the ones most relevant to this book:
...
<div data-menu="closed"> <1>
  <button type="button" data-menu-button>Options</button> <2>
...
This is based on the Menu Button example footnote:[https://www.w3.org/WAI/ARIA/apg/patterns/menubutton/] from the cite:[ARIA Authoring Practices Guide]. We haven't made the menu work yet, so these attributes are wrong for now.
...
The use of libraries in client-side web programming poses unique constraints, as code is downloaded every time the application is loaded (with caching).

Many JS frameworks have an entourage of wrappers for common libraries.
These "react-foo-bar" libraries serve as adapters that resolve the mismatch between the framework's view of the DOM and that of the foo-bar library.
The mismatch, in turn, occurs because modern frameworks rely on the assumption that they control the DOM, or at least a subtree of it.

In a HDA, it is a lot easier for libraries to perform their DOM manipulations freely.
However, the following invariants need to be maintained for best results:

[.text-center]
The app can be restored fully from an HTML snapshot of the DOM (i.e. it should be possible to set `document.body.innerHTML = document.body.innerHTML` and restore the app to a working state from there).


=== Integration through custom events

Libraries which use custom events over methods and callbacks are easier to integrate with HDA technologies, as these events can be dispatched or listened to without glue code.
Compare the following two examples (using Alpine):
...
<button @click="
  FooDialog.confirm({
    ...
    onClose: (event) => {
      if (event.yes) {
        htmx.ajax('DELETE', '/contacts/34', '#delete-button')
      }
    }
  })
">
...
[source,html]
----
<button id="delete-button" hx-delete="/contacts/34" hx-trigger="foodialog:close[yes]"
  @click="FooDialog.confirm($el, { title: 'Delete this contact?' })">
----

Unfortunately, many libraries expect you to pass a callback in the first style.
In these cases, it can be useful to add a simple callback that just dispatches a custom event and move logic to the event listener.
